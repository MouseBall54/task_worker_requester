...
폴더+Recipe 단위 진행률은 SQLite에 증감식으로 저장된 상태 카운터를 사용하므로 전체 작업을 다시 읽지 않습니다. 이미지가 없는 대기열은 이미지 없음 상태로 표시됩니다. 같은 세션의 동일한 폴더 경로와 Recipe 경로 조합만 중복으로 차단됩니다.""",
    ...
    HelpTopic(
        ...
        body="""왼쪽 즐겨찾기 영역은 자주 사용하는 폴더 경로를 프로그램 재실행 후에도 유지합니다. `즐겨찾기 관리` 창에는 Root 경로, 색인 범위, 상태, 색인됨, 대기, 오류, 마지막 완료가 표시됩니다. 기본은 전체 계층 완전 색인이며 필요할 때만 하위 5계층 또는 색인 제외를 선택할 수 있습니다. 전체 재색인, 증분 갱신, 색인 일시정지/재개와 오류 보기를 지원합니다. 즐겨찾기 항목을 누르면 해당 경로로 즉시 이동합니다.
