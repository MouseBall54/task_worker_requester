...
2. 폴더 추가를 누르면 선택한 폴더 자체를 작업 대상으로 등록합니다. 설정의 scan_mode가 direct이면 선택 폴더 바로 아래의 이미지만 대상으로 삼고, recursive이면 하위 폴더까지 탐색합니다. 동일 경로가 이미 등록되어 있으면 중복 추가하지 않고 진행중/대기 또는 완료된 폴더 중 어디에 존재하는지 테이블 팝업으로 안내합니다.

3. sub_folder 추가를 누르면 선택한 폴더 아래에서 이미지가 들어 있는 하위 폴더들을 작업 단위로 등록합니다. 여러 하위 폴더를 폴더 단위 진행 현황에서 따로 추적하려는 경우에 사용합니다. 중복 폴더가 여러 개면 탐색 완료 후 한 번의 테이블 팝업으로 모아서 안내합니다.
...
9. 폴더 추가 순서는 SQLite folders.position에 폴더별 전송 우선순위로 저장되고 진행중/대기 표의 우선순위 열에는 1부터 시작하는 번호로 표시됩니다. 아직 실행되지 않은 폴더를 맨 위, 위, 아래, 맨 아래로 이동하거나 보류/해제할 수 있으며 변경된 표 순서가 실제 publish 순서가 됩니다. 보류 폴더는 전체 모수에 포함되지만 해제 전까지 publish되지 않습니다. 스캔·전송·처리가 시작된 폴더는 순서와 보류 상태를 변경할 수 없습니다.
...
폴더를 추가할 때 SQLite folders.position은 기존 마지막 우선순위 다음 값으로 자동 배정됩니다. 화면의 우선순위는 position+1로 표시됩니다. 맨 위, 위, 아래, 맨 아래 이동 버튼은 position을 한 트랜잭션에서 연속된 값으로 다시 저장합니다. 여러 폴더를 선택해 이동하면 선택 폴더 사이의 상대 순서를 유지하며 실제 MQ 발행도 이 position 순서를 사용합니다. 이미 실행 중인 폴더의 position은 고정하고 아직 실행되지 않은 폴더의 우선순위 슬롯만 재배치합니다.
