...
9. 가운데 표의 진행 중/대기 폴더와 완료된 폴더를 선택하면 오른쪽 상세 상태 탭에서 해당 폴더의 이미지 작업을 확인할 수 있습니다. 이미지 파일명에서 확장자를 제외한 부분이 숫자이면 1, 2, 10과 같은 숫자 오름차순으로 표시됩니다. MQ 버튼은 선택 이미지의 예상 payload, 실제 publish payload, 수신 payload를 비교하는 용도입니다.""",
    ),
    HelpTopic(
        title="폴더 스캔 방식",
