...
4. Recipe는 worker에 전달할 RECIPE_PATH입니다. Recipe 선택 메뉴에서 하나 이상을 체크할 수 있고, 선택 경로 영역에는 Recipe별 alias와 path가 각각 표시됩니다. 폴더 추가 시점의 선택마다 폴더 단위 진행 현황에 별도 행이 생성되며 각 행은 하나의 Recipe만 가집니다. 이미지 N개와 Recipe M개는 N×M개의 고유 request로 등록됩니다. 전송 중에도 Recipe 선택을 바꿀 수 있으며 변경한 선택은 이후 추가하는 작업에만 적용되고 기존 작업에는 소급 적용되지 않습니다. 로컬에서 recipe 파일이 보이지 않아도 payload에는 설정 문자열이 그대로 전송될 수 있으므로, worker가 접근할 수 있는 경로인지 확인해야 합니다.
...
11. 가운데 표의 진행 중/대기 폴더와 완료된 폴더를 선택하면 오른쪽 상세 상태 탭에서 해당 폴더의 이미지 작업을 확인할 수 있습니다. 두 표의 헤더 경계를 드래그하면 상태·폴더·Recipe·타임아웃을 포함한 모든 열 너비를 직접 조절할 수 있으며, 표별 열 너비는 프로그램을 종료해도 유지됩니다. 이미지 파일명에서 확장자를 제외한 부분이 숫자이면 1, 2, 10과 같은 숫자 오름차순으로 표시됩니다. MQ 버튼은 선택 이미지의 예상 payload, 실제 publish payload, 수신 payload를 비교하는 용도입니다.""",
    ),
    HelpTopic(
        title="폴더 스캔 방식",
...
폴더+Recipe 단위 진행률은 SQLite에 증감식으로 저장된 상태 카운터를 사용하므로 전체 작업을 다시 읽지 않습니다. 이미지가 없는 대기열은 이미지 없음 상태로 표시됩니다. 같은 세션의 동일한 폴더 경로와 Recipe 경로 조합만 중복으로 차단됩니다.""",
    ...
    HelpTopic(
        ...
        body="""왼쪽 즐겨찾기 영역은 자주 사용하는 폴더 경로를 프로그램 재실행 후에도 유지합니다. `즐겨찾기 관리` 창에는 Root 경로, 색인 범위, 상태, 색인됨, 대기, 오류, 마지막 완료가 표시됩니다. 신규 Root는 하위 5계층이 기본이며 하위 3·4·5·6계층, 전체 계층 또는 색인 제외를 선택할 수 있습니다. 전체 재색인, 증분 갱신, 색인 일시정지/재개와 오류 보기를 지원합니다. 즐겨찾기 항목을 누르면 해당 경로로 즉시 이동합니다.

즐겨찾기 목록 아래의 통합 입력창은 실제 폴더 경로 이동과 즐겨찾기 Root 검색을 함께 지원합니다. 유효한 경로를 입력하고 Enter 또는 `검색`을 누르면 트리로 이동하며, 일반 검색어를 입력하면 SQLite 캐시의 폴더명과 전체 경로를 검색합니다. 검색은 진행 중인 전체 색인을 취소하거나 파일 시스템을 다시 순회하지 않습니다. 결과는 200개 단위로 표시하며 `검색 결과 더 보기`로 이어서 볼 수 있습니다. 결과를 누를 때 실제 존재 여부를 다시 확인합니다.

색인 대상은 별도 SQLite 대기열에 500개 단위로 저장되므로 수만 개 폴더를 처리하다 프로그램이 종료돼도 다음 실행에서 이어집니다. 대기 0, 미처리 오류 0, 세대 정리 완료 조건을 모두 만족할 때만 완료 상태가 됩니다. 색인 일시정지 후에도 이미 표시된 캐시 결과는 유지됩니다. Windows 긴 경로는 탐색 시 확장 경로로 처리하고 접근 권한·네트워크·I/O 오류는 관리 창의 오류 목록에 남깁니다.
