...
```cmd
python -c "import os,sqlite3; from pathlib import Path; c=sqlite3.connect(Path(os.environ['DB']).as_uri()+'?mode=ro',uri=True); print('STATUS | COUNT'); [print(status,count,sep=' | ') for status,count in c.execute('SELECT status,COUNT(*) FROM tasks GROUP BY status ORDER BY status')]"
```
...
폴더는 프로그램의 진행 중/대기 표와 같은 등록 순서인 `position` 순으로 출력한다.

```cmd
python -c "import os,sqlite3; from pathlib import Path; c=sqlite3.connect(Path(os.environ['DB']).as_uri()+'?mode=ro',uri=True); q='SELECT position,scan_state,total_count,pending_count,claimed_count,sent_count,running_count,success_count,fail_count,timeout_count,error_count,cancelled_count,folder_path FROM folders ORDER BY position'; print('POS | SCAN | TOTAL | PENDING | CLAIMED | SENT | RUNNING | SUCCESS | FAIL | TIMEOUT | ERROR | CANCELLED | FOLDER'); [print(*r,sep=' | ') for r in c.execute(q)]"
```
...
```cmd
python -c "import os,sqlite3; from pathlib import Path; c=sqlite3.connect(Path(os.environ['DB']).as_uri()+'?mode=ro',uri=True); q='SELECT status,recipe_alias,image_path,sent_at,completed_at FROM tasks ORDER BY rowid DESC LIMIT 20'; print('STATUS | RECIPE | IMAGE | SENT_AT | COMPLETED_AT'); [print(*r,sep=' | ') for r in c.execute(q)]"
```
...
```cmd
python -c "import os,sqlite3; from pathlib import Path; c=sqlite3.connect(Path(os.environ['DB']).as_uri()+'?mode=ro',uri=True); q='SELECT t.request_id,t.status,t.recipe_alias,t.image_path FROM tasks t JOIN folders f ON f.session_id=t.session_id AND f.folder_path=t.folder_path WHERE t.status=? ORDER BY f.position,t.rowid LIMIT 100'; print('REQUEST_ID | STATUS | RECIPE | IMAGE'); [print(*r,sep=' | ') for r in c.execute(q,(os.environ['STATUS'],))]"
```
...
```cmd
python -c "import os,sqlite3; from pathlib import Path; c=sqlite3.connect(Path(os.environ['DB']).as_uri()+'?mode=ro',uri=True); q='SELECT status,request_id,image_path,error_message,completed_at FROM tasks WHERE status IN (?,?,?) ORDER BY rowid DESC LIMIT 100'; print('STATUS | REQUEST_ID | IMAGE | ERROR | COMPLETED_AT'); [print(*r,sep=' | ') for r in c.execute(q,('FAIL','TIMEOUT','ERROR'))]"
```
...
```cmd
python -c "import os,sqlite3; from pathlib import Path; c=sqlite3.connect(Path(os.environ['DB']).as_uri()+'?mode=ro',uri=True); total,done=c.execute('SELECT COALESCE(SUM(total_count),0),COALESCE(SUM(success_count+fail_count+timeout_count+error_count+cancelled_count),0) FROM folders').fetchone(); pending_scan=c.execute('SELECT EXISTS(SELECT 1 FROM folders WHERE scan_state IN (?,?))',('WAITING','SCANNING')).fetchone()[0]; print('전체 모수 확정:',not bool(pending_scan)); print('완료/전체:',str(done)+'/'+str(total)); print('진행률:',f'{done/total*100:.1f}%' if total else '0.0%')"
```
...
```cmd
python -c "import os,sqlite3,csv; from pathlib import Path; c=sqlite3.connect(Path(os.environ['DB']).as_uri()+'?mode=ro',uri=True); q='SELECT request_id,status,folder_path,image_path,recipe_alias,recipe_path,sent_at,completed_at,error_message FROM tasks ORDER BY rowid'; rows=c.execute(q); f=open('task_status.csv','w',newline='',encoding='utf-8-sig'); w=csv.writer(f); w.writerow([d[0] for d in rows.description]); w.writerows(rows); f.close(); print('생성 완료:',Path('task_status.csv').resolve())"
```
