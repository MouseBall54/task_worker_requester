...
- 즐겨찾기 Root와 폴더명 검색 인덱스는 작업 상태와 분리된 `%APPDATA%\IPDK_plus\runtime\folder_index.sqlite3`에 저장됩니다. 기본 전체 계층 색인은 SQLite 영속 대기열을 사용해 중단 후 재개되며, 검색은 파일 시스템 재탐색 없이 캐시만 페이지 단위로 조회합니다. 선택적으로 하위 5계층 또는 색인 제외 정책을 지정할 수 있습니다. 자세한 상태·오류·운영 방법은 [즐겨찾기 Root 및 폴더 검색 가이드](.\docs\folder_search_index.md)를 참고하세요.
