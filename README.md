...
- 상단 `Worker Count`를 클릭하면 요청 queue consumer를 연결 IP별로 확인할 수 있습니다. IP와 장비명은 `%APPDATA%\IPDK_plus\worker_nodes.yaml`에서 관리하며 팝업에서 추가·수정·삭제 후 저장할 수 있습니다. RabbitMQ Management 플러그인과 HTTP API 조회 권한이 필요하고, Management API 주소와 앱에 설정된 RabbitMQ 계정을 사용합니다. 비 TLS 네트워크에서는 API 주소에 HTTPS를 설정하세요.
...
- 설치 프로그램 파일명은 버전을 포함한 `IPDK_plusSetup_26.8.18.exe` 형식입니다.
